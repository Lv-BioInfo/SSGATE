...
expression_matrix <- read.csv('Monocle3_folder/expression_matrix_ssmidgate.csv', row.names = 1)
metadata <- read.csv('Monocle3_folder/metadata_ssmidgate.csv', row.names = 1)
gene_metadata <- read.csv('Monocle3_folder/gene_metadata_ssmidgate.csv', row.names = 1)
...
fuse_embed <- read.csv('Monocle3_folder/Fuse_embed_ssmidgate.csv', row.names = 1)
umap <- read.csv('Monocle3_folder/umap_ssmidgate.csv', row.names = 1)
...
# plot_cells(cds, label_groups_by_cluster=FALSE,  color_cells_by = "celltype_spatialfusor_leiden")
```


```{r}
# Convert cell type information to character and add color
colData(cds)$celltype_SSMI_DGATE_leiden <- as.character(colData(cds)$celltype_SSMI_DGATE_leiden)
unique_cell_types <- sort(unique(colData(cds)$celltype_SSMI_DGATE_leiden))
colors <- c('#1f77b4', '#ff7f0e', '#279e68', '#d62728', '#aa40fc', '#8C564B', '#8c564b')
color_mapping <- setNames(colors, unique_cell_types)
colData(cds)$color <- color_mapping[colData(cds)$celltype_SSMI_DGATE_leiden]
```
...
p <- plot_cells(cds,
           color_cells_by = "celltype_SSMI_DGATE_leiden",
